...
//! from track geometry, then packs them into the optimizer's decision
//! variable format via `CollocationOptimizer::initial_guess_from_profiles`.
...
/// the output, and resamples from [`N_FIXED`] to `n_nodes` points.
///
...
///
/// Extracts track features, runs neural network inference, resamples
/// the predictions from [`N_FIXED`] to `config.n_nodes`, and packs them
/// into the optimizer's decision variable vector using
/// [`CollocationOptimizer::initial_guess_from_profiles`].
///
/// The network outputs raw speed (m/s) and offset (m) predictions in the
/// same space as the training targets; no denormalization is needed.
///
/// Returns `None` if inference fails or the output contains NaN/Inf values.
...
pub fn ml_initial_guess(
    ...
    let profiles = ml_predict_profiles(net, track, config.n_nodes, config.v_min)?;
...
/// Wrapper that holds a loaded network and provides warmstart generation.
///
...
impl MlWarmstart {
    /// Create from an already-loaded network.
    pub fn from_net(net: RacelineNet) -> Self {
        Self { net }
    }
    ...
    pub fn predict_profiles(
        ...
        ml_predict_profiles(&self.net, track, n_nodes, v_min)
    }
    ...
    pub fn generate(
        ...
        ml_initial_guess(&self.net, track, config, car)
    }
}
...
mod tests {
    ...
    fn test_ml_predict_profiles_shape() {
        ...
        let n_nodes = 50;
        let profiles = ml_predict_profiles(&net, &track, n_nodes, 5.0);
    ...
    fn test_ml_initial_guess_shape() {
        ...
        let guess = ml_initial_guess(&net, &track, &config, &car);
    ...
    fn test_ml_initial_guess_no_nan() {
        ...
        let car = CarParams::default();
        let x = ml_initial_guess(&net, &track, &config, &car).expect("guess");
        assert!(
